...
This module provides various strategies for detecting changes in data for incremental processing.
Each strategy is designed to handle different scenarios where traditional timestamp columns
...
class SequenceStrategy(DeltaStrategy):
    ...
    def get_incremental_data(self, source_table: Table, destination_table: Table, 
                           strategy_config: Dict[str, Any]) -> Table:
        """Get records with sequence numbers higher than the maximum in destination."""
...
class FullScanStrategy(DeltaStrategy):
    """Strategy that compares all records to detect changes (expensive but comprehensive)."""
