...
class UserAIContextRules(models.Model):
    ...
    user = models.OneToOneField(User, on_delete=models.CASCADE, related_name="ai_context_rules")
...
class ProjectAIContextRules(models.Model):
    """Model for storing project-specific AI context rules (single entry per
    project)"""

    project = models.OneToOneField(ProjectDetails, on_delete=models.CASCADE, related_name="ai_context_rules")
    context_rules = models.TextField(default="", blank=True)
    created_by = models.ForeignKey(User, on_delete=models.CASCADE, related_name="created_project_ai_context_rules")
    updated_by = models.ForeignKey(User, on_delete=models.CASCADE, related_name="updated_project_ai_context_rules")
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
