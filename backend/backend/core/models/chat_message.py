...
class ChatMessage(BaseModel):
    ...
    chat_message_id = models.UUIDField(
        primary_key=True, default=uuid.uuid4, editable=False, help_text="Unique identifier for this ChatMessage."
    )
    ...
    prompt = models.CharField(
        max_length=65000, help_text="Text input or question from the user.", null=False, blank=False
    )

    response = models.JSONField(null=True, blank=True, help_text="JSON response generated for this message.")
    ...
    response_time = models.PositiveIntegerField(
        help_text="Time taken in milliseconds to generate the response.", null=False, blank=False, default=0
    )

    thought_chain = models.JSONField(
        default=list, null=True, blank=True, help_text="List of thought processes before generating the response."
    )
    ...
    prompt_error_message = models.JSONField(null=True, blank=True, help_text="Error message for prompt processing.")
    ...
    generated_models = models.JSONField(
        default=list, null=True, blank=True, help_text="List of generated models after transformation Applied."
    )

    transformation_error_message = models.JSONField(
        null=True, blank=True, help_text="Error message for transformation processing."
    )
    ...
    FEEDBACK_CHOICES = [("0", "Neutral"), ("P", "Positive"), ("N", "Negative")]
    ...
    feedback_timestamp = models.DateTimeField(null=True, blank=True, help_text="When the feedback was provided.")

    feedback_comment = models.TextField(null=True, blank=True, help_text="Optional comment provided with feedback.")
