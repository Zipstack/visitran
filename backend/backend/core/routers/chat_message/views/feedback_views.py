...
class ChatMessageFeedbackView(APIView):
    ...
    def post(self, request, chat_message_id, project_id=None, chat_id=None, **kwargs):
        ...
        try:
            ...
            if not org_id:
                return Response(
                    {"error": BackendErrorMessages.ORGANIZATION_REQUIRED}, status=status.HTTP_400_BAD_REQUEST
                )

            # Find the chat message
            chat_message = ChatMessage.objects.filter(chat_message_id=chat_message_id).first()

            if not chat_message:
                return Response(
                    {"error": BackendErrorMessages.CHAT_MESSAGE_NOT_FOUND}, status=status.HTTP_404_NOT_FOUND
                )
            ...
            if serializer.is_valid():
                ...
                chat_message.save(update_fields=["has_feedback", "feedback", "feedback_comment", "feedback_timestamp"])

                logging.info(
                    f"Feedback submitted for chat message {chat_message_id}: " f"feedback={chat_message.feedback}"
                )

                return Response(
                    {"success": True, "message": "Feedback submitted successfully"}, status=status.HTTP_200_OK
                )

            # Use INVALID_FEEDBACK_FORMAT for serializer validation errors
            return Response({"error": BackendErrorMessages.INVALID_FEEDBACK_FORMAT}, status=status.HTTP_400_BAD_REQUEST)

        except Exception as e:
            logging.exception(f"Error submitting feedback for chat message {chat_message_id}")
            error_message = BackendErrorMessages.FEEDBACK_SUBMISSION_FAILED.format(chat_message_id=chat_message_id)
    ...
    def get(self, request, chat_message_id, project_id=None, chat_id=None, **kwargs):
        ...
        try:
            ...
            error_message = BackendErrorMessages.FEEDBACK_RETRIEVAL_FAILED.format(chat_message_id=chat_message_id)
            return Response({"error": error_message}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
