...
def user_ai_context_rules(request: Request) -> Response:
    ...
    try:
        ...
        if request.method == HTTPMethods.GET:
            ...
            context_rules, created = UserAIContextRules.objects.get_or_create(user=user, defaults={"context_rules": ""})
            ...
            context_rules, created = UserAIContextRules.objects.get_or_create(
                user=user, defaults={"context_rules": context_rules_text}
            )
...
def project_ai_context_rules(request: Request, project_id: str) -> Response:
    ...
    try:
        try:
            ...
            return Response(
                {
                    "error_message": BackendErrorMessages.AI_CONTEXT_RULES_INVALID_PROJECT.format(
        ...
        if request.method == HTTPMethods.GET:
            ...
            context_rules, created = ProjectAIContextRules.objects.get_or_create(
                project=project, defaults={"context_rules": context_rules_text, "created_by": user, "updated_by": user}
            )
