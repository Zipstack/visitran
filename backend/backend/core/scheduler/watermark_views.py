...
def detect_watermark_columns(request, project_id):
    ...
    try:
        ...
        if not environment_id:
            return Response({"error": "environment_id is required"}, status=status.HTTP_400_BAD_REQUEST)
        ...
        try:
            ...
            return Response({"error": "Project or environment not found"}, status=status.HTTP_404_NOT_FOUND)
