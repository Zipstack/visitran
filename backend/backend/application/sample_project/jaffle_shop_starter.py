...
class JaffleShopProjectStarter(SampleProject):

    def __init__(self):
        super().__init__()
        self._base_path = None
        self._prefix = "Starter"
    ...
    def master_db_name(self):
        return "jaffleshop_starter_master"
    ...
    def database_name(self) -> str:
        return self.sanitize_name(f"{self.org_id}_{self._user_id}_js_db_s_{self.timestamp_str}")

    @property
    def user_name(self) -> str:
        return f"dbuser_{self.org_id}_{self._user_id}_js_s"[:63]

    @property
    def postgres_connection_details(self):
        return {
            "name": self.connection_name,
            "description": f"{JaffleShopProjectConstants.connection_desc}",
