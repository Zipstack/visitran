...
class TestFormulaSQLDateTime:
    ...
    def setup(self, mysql_sakila_db):
        ...
        host = db.ip
        port = db.port
        self.connection_mysql = ibis.mysql.connect(
            host=host, port=port, user=user, password=password, database="sakila"
        )
    ...
    def test_datediff(self):
        formula = FormulaSQL(self.payment, "test_col1", '=DATEDIFF(DATE(2023,6,29),DATE(2022,6,29),"D")')
        payment_x = self.payment.mutate(formula.ibis_column())
        row = payment_x["payment_id", "customer_id", "amount", "payment_date", "test_col1"].head().execute().iloc[0]
        assert row["test_col1"] == 365
        formula = FormulaSQL(self.payment, "test_col1", '=DATEDIFF(DATE(2023,6,29),DATE(2022,6,29),"M")')
        payment_x = self.payment.mutate(formula.ibis_column())
        row = payment_x["payment_id", "customer_id", "amount", "payment_date", "test_col1"].head().execute().iloc[0]
        assert row["test_col1"] == 12
        formula = FormulaSQL(self.payment, "test_col1", '=DATEDIFF(DATE(2023,6,29),DATE(2022,6,29),"Y")')
        payment_x = self.payment.mutate(formula.ibis_column())
        row = payment_x["payment_id", "customer_id", "amount", "payment_date", "test_col1"].head().execute().iloc[0]
    ...
    def test_datetimediff(self):
        formula = FormulaSQL(
            self.payment, "test_col1", '=DATETIMEDIFF(DATETIME(2023,6,29,0,0,0),DATETIME(2022,6,29,0,0,0),"D")'
        )
        payment_x = self.payment.mutate(formula.ibis_column())
        row = payment_x["payment_id", "customer_id", "amount", "payment_date", "test_col1"].head().execute().iloc[0]
        assert row["test_col1"] == 365
        formula = FormulaSQL(
            self.payment, "test_col1", '=DATETIMEDIFF(DATETIME(2023,6,29,0,0,0),DATETIME(2022,6,29,0,0,0),"M")'
        )
        payment_x = self.payment.mutate(formula.ibis_column())
        row = payment_x["payment_id", "customer_id", "amount", "payment_date", "test_col1"].head().execute().iloc[0]
        assert row["test_col1"] == 12
        formula = FormulaSQL(
            self.payment, "test_col1", '=DATETIMEDIFF(DATETIME(2023,6,29,0,0,0),DATETIME(2022,6,29,0,0,0),"Y")'
        )
        payment_x = self.payment.mutate(formula.ibis_column())
        row = payment_x["payment_id", "customer_id", "amount", "payment_date", "test_col1"].head().execute().iloc[0]
        assert row["test_col1"] == 1
