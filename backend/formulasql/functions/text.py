...
class Text(Base):
    ...
    def join(table, node, data_types, inter_exps):
        ...
        oe = FormulaSQLUtils.build_ibis_expression(table, data_types, inter_exps, node["inputs"][len - 1]).cast(
            "string"
        )
        e = oe
        for i in range(len - 1, 1, -1):
            e = FormulaSQLUtils.build_ibis_expression(table, data_types, inter_exps, node["inputs"][i - 1]).cast(
                "string"
            )
            e = s0.join([e, oe])
            oe = e
    ...
    def translate(table, node, data_types, inter_exps):
        ...
        e = FormulaSQLUtils.build_ibis_expression(table, data_types, inter_exps, node["inputs"][0]).cast("string")
        from_chars = FormulaSQLUtils.build_ibis_expression(table, data_types, inter_exps, node["inputs"][1]).cast(
            "string"
        )
        to_chars = FormulaSQLUtils.build_ibis_expression(table, data_types, inter_exps, node["inputs"][2]).cast(
            "string"
        )
    ...
    def split(table, node, data_types, inter_exps):
        ...
        e = FormulaSQLUtils.build_ibis_expression(table, data_types, inter_exps, node["inputs"][0]).cast("string")
        delimiter = FormulaSQLUtils.build_ibis_expression(table, data_types, inter_exps, node["inputs"][1]).cast(
            "string"
        )
        e = e.split(delimiter)
