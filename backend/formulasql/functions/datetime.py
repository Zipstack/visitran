...
class DateTime:
    ...
    def date(table, node, data_types, inter_exps):
        if node['inputs'].__len__() == 3:
            ...
            e = ibis.literal(
                datetime.datetime.strptime(node['inputs'][0], node['inputs'][1]))
            data_types[node['outputs'][0]] = 'date'
    ...
    def datetime(table, node, data_types, inter_exps):
        if node['inputs'].__len__() == 6:
            e = ibis.literal(datetime.datetime(DateTime.__num(node['inputs'][0]), DateTime.__num(node['inputs'][1]),
                                               DateTime.__num(node['inputs'][2]), DateTime.__num(node['inputs'][3]),
                                               DateTime.__num(node['inputs'][4]), DateTime.__num(node['inputs'][5])))
            data_types[node['outputs'][0]] = 'datetime'
        elif node['inputs'].__len__() == 1:
