...
const EnvironmentBadge = ({ type, name }) => {
  ...
  return (
    <Space direction="vertical" size={1}>
      <Tag color={c.color} icon={c.icon} style={{ fontWeight: 700, fontSize: 10, letterSpacing: 0.4, margin: 0 }}>
        {c.label}
      </Tag>
      <Text type="secondary" style={{ fontSize: 11, fontFamily: "monospace" }}>{name}</Text>
    </Space>
  );
};
  ...
  return (
    <Space direction="vertical" size={1}>
      <Space size={4}>
        ...
        {expression && <Text code style={{ fontSize: 11 }}>{expression}</Text>}
      </Space>
...
const JobListTable = memo(
  ({
    ...
    const handleSwitchSchedular = async (item, checked) => {
      try {
        const { user_task_id, project, periodic_task_details, description, task_name, task_type, environment } = item;
    ...
    const handleDelete = (record) => {
      setDelTaskDetail({ projectId: record.project.id, taskId: record.periodic_task_details.id });
    ...
    const handleRun = async (projectId, taskId) => {
      ...
      try {
        ...
        setLoading((prev) => { const { [taskId]: _, ...rest } = prev; return rest; });
      }
    };
    ...
    const columns = useMemo(() => [
      ...
      },
      {
        ...
        key: "environment",
        render: (env) => env ? <EnvironmentBadge type={env.type} name={env.name} /> : <Text type="secondary">—</Text>,
      },
      {
        title: "Schedule",
        key: "schedule",
        ...
        render: (_, record) => {
          ...
          return (
            <Space direction="vertical" size={2}>
              <Tag
                icon={isSuccess ? <CheckCircleFilled /> : isFailed ? <CloseCircleFilled /> : isRunning ? <SyncOutlined spin /> : null}
              ...
              {record.task_completion_time && (
                <Tooltip title={new Date(record.task_completion_time).toISOString()}>
                  <Text style={{ fontSize: 12 }}>{formatDateTime(record.task_completion_time)}</Text>
                </Tooltip>
              )}
        ...
        render: (text, record) => {
          if (!text) {
            if (!record.periodic_task_details?.enabled) {
              return <Space size={4}><PauseCircleOutlined style={{ color: token.colorTextSecondary }} /><Text type="secondary">Paused</Text></Space>;
            }
          ...
          return (
            <Space size={8}>
              <Switch
                ...
                size="small"
              />
              <Text type="secondary" style={{ fontSize: 12 }}>{enabled ? "Enabled" : "Paused"}</Text>
            </Space>
          );
        },
      },
          ...
          <Space size={4}>
            <Tooltip title="Run now">
              <Button type="text" size="small"
                icon={loading[record.user_task_id] ? <LoadingOutlined /> : <PlayCircleOutlined style={{ color: token.colorPrimary }} />}
    ...
    return (
      <Table
        ...
        pagination={false}
        locale={{
          emptyText: <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description="No jobs created yet" />,
        }}
      />
    );
  }
