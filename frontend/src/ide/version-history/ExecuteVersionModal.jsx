  ...
  const handleExecute = useCallback(async () => {
    ...
    try {
      ...
      if (res?.status === "success") {
        setResult({
          success: true,
          message: res.data?.message || "All transformations completed.",
        });
        onExecuteSuccess?.();
      } else {
        setResult({
          success: false,
          error_message: res?.error_message || "Execution failed.",
        });
      }
    } catch (err) {
      const errMsg =
  ...
  const renderResult = () => {
    ...
    return (
      <Result
        ...
        subTitle="One or more transformations failed."
        extra={
          <Collapse
            ghost
            items={[
